...
from p2fx import utils as p2fx_utils
...
class SmdImporterEx(vs_import_smd.SmdImporter):
	bl_idname = "p2fx.smd_importer_ex"
	...
	def execute(self, context):
		...
		GP2grImporter.smd = self.smd
		return {'FINISHED'}
	
	def readPolys(self):
		if GP2grImporter.onlyBones:
			return
...
def ReadQAngle(file):
	...
	return p2fx_utils.QAngle(x,y,z)
...
def ReadP2grVersion(file):
	buf = file.read(15)
	if len(buf) < 15:
		return None
	
	cmp = b"p2fxGameRecord\0"
	...
	def Update(self,curTime,interKey):
		if((self.lastTime is not None) and ((curTime is None) or (self.lastTime < curTime))):
			...
			if self.visible is not None:
				if interKey:
					p2fx_utils.AppendInterKeys_Visible(self.lastTime, 0 if self.visible else 1, self.visibilityFrames)
			...
			if self.location is not None:
				...
				if interKey:
					p2fx_utils.AppendInterKeys_Location(self.lastTime, self.location, self.locationXFrames, self.locationYFrames, self.locationZFrames)
			...
			if self.rotation is not None:
				...
				if interKey:
					p2fx_utils.AppendInterKeys_Rotation(self.lastTime, self.rotation, self.rotationWFrames, self.rotationXFrames, self.rotationYFrames, self.rotationZFrames)
			...
			if self.scale is not None:
				if interKey:
					p2fx_utils.AppendInterKeys_Location(self.lastTime, self.scale, self.scaleXFrames, self.scaleYFrames, self.scaleZFrames)
			...
			if self.bones is not None:
				for idx,i in enumerate(self.bones):
					...
					if interKey:
						p2fx_utils.AppendInterKeys_Location(self.lastTime, bone.location, self.boneLocationXFrames[i], self.boneLocationYFrames[i], self.boneLocationZFrames[i])
						p2fx_utils.AppendInterKeys_Rotation(self.lastTime, renderRotQuat, self.boneRotationWFrames[i], self.boneRotationXFrames[i], self.boneRotationYFrames[i], self.boneRotationZFrames[i])
						p2fx_utils.AppendInterKeys_Location(self.lastTime, bone.scale, self.boneScaleXFrames[i], self.boneScaleYFrames[i], self.boneScaleZFrames[i])
	...
	def Update(self,curTime,interKey):
		if((self.lastTime is not None) and ((curTime is None) or (self.lastTime < curTime))):
			...
			if self.lens is not None:
				if interKey:
					p2fx_utils.AppendInterKeys_Value(self.lastTime, self.lens, self.lensFrames)
...
class P2grImporter(bpy.types.Operator, vs_utils.Logger):
	bl_idname = "p2fx.p2grimporter"
	bl_label = "P2FX p2fxGameRecord (.p2gr)"
	bl_options = {'REGISTER', 'UNDO', 'PRESET'}
	
	# Properties used by the file browser
	filepath: bpy.props.StringProperty(subtype="FILE_PATH")
	filter_glob: bpy.props.StringProperty(default="*.p2gr", options={'HIDDEN'})
	...
	keyframeInterpolation: bpy.props.EnumProperty(
		...
		description="Constant recommended for beginners." if p2fx_utils.NEWER_THAN_290 else "Constant recommended for beginners. Advanced users can choose Bezier for significantly faster import times.",
		items=[
			('CONSTANT', "Constant (recommended)", "No interpolation"),
			('LINEAR', "Linear", "Linear interpolation"),
			('BEZIER', "Bezier" if p2fx_utils.NEWER_THAN_290 else "Bezier (fast import)", "Smooth interpolation"),
		],
		default='CONSTANT'
	)
	...
	def execute(self, context):
		...
		print("P2GR import finished in %.4f sec." % (time.time() - time_start))
		return {'FINISHED'}
	...
	def importModel(self, context, modelHandle):
		...
		def makeModelName(modelHandle):
			...
			name = "p2fx." +str(modelHandle.objNr)+ " " + name
			return name
		...
		if modelData is None:
			...
			GP2grImporter.smd = None
			GP2grImporter.onlyBones = self.onlyBones
			modelData = None

			try:
				bpy.ops.p2fx.smd_importer_ex(filepath=filePath, doAnim=False)
				modelData = ModelData(GP2grImporter.smd)
	...
	def readP2gr(self,context):
		...
		try:
			...
			version = ReadP2grVersion(file)
			
			if version is None:
				self.error('Invalid file formatasdasd.')
				return result
			...
			timeConverter = P2grTimeConverter(context)
			currentTime = timeConverter.GetTime()
			dictionary = P2grDictionary()
			...
			while True:
				...
				if node0 is None:
				...
				elif 'p2fxFrame' == node0:
					frameTime = ReadFloat(file)
					
					timeConverter.Frame(frameTime)
					currentTime = timeConverter.GetTime()
					
					p2fxHiddenOffset = ReadInt(file)
					if p2fxHiddenOffset:
						curOffset = file.tell()
						file.seek(p2fxHiddenOffset -4, 1)
				...
				elif 'p2fxFrameEnd' == node0:
					timeConverter.FrameEnd()
					
				elif 'p2fxHidden' == node0:
					# skipped, because will be handled earlier by p2fxHiddenOffset
					...
					if camData is None:
						camData = self.createCamera(context,"p2fxCam")
			...
			for modelHandle in modelHandles:
				...
				if modelCamData is not None:
					...
					p2fx_utils.AddKeysList_Location(self.keyframeInterpolation, curves[0].keyframe_points, curves[1].keyframe_points, curves[2].keyframe_points, modelCamData.locationXFrames, modelCamData.locationYFrames, modelCamData.locationZFrames)
					p2fx_utils.AddKeysList_Rotation(self.keyframeInterpolation, curves[3].keyframe_points, curves[4].keyframe_points, curves[5].keyframe_points, curves[6].keyframe_points, modelCamData.rotationWFrames, modelCamData.rotationXFrames, modelCamData.rotationYFrames, modelCamData.rotationZFrames)
					p2fx_utils.AddKeysList_Value(self.keyframeInterpolation, curves[7].keyframe_points, modelCamData.lensFrames)
				...
				p2fx_utils.AddKeysList_Visible(curves[0].keyframe_points, modelHandle.visibilityFrames)
				p2fx_utils.AddKeysList_Location(self.keyframeInterpolation, curves[1].keyframe_points, curves[2].keyframe_points, curves[3].keyframe_points, modelHandle.locationXFrames, modelHandle.locationYFrames, modelHandle.locationZFrames)
				p2fx_utils.AddKeysList_Rotation(self.keyframeInterpolation, curves[4].keyframe_points, curves[5].keyframe_points, curves[6].keyframe_points, curves[7].keyframe_points, modelHandle.rotationWFrames, modelHandle.rotationXFrames, modelHandle.rotationYFrames, modelHandle.rotationZFrames)
				p2fx_utils.AddKeysList_Location(self.keyframeInterpolation, curves[8].keyframe_points, curves[9].keyframe_points, curves[10].keyframe_points, modelHandle.scaleXFrames, modelHandle.scaleYFrames, modelHandle.scaleZFrames)
				...
				currentFrames = 0
				for i in modelHandle.boneLocationXFrames:
					p2fx_utils.AddKeysList_Location(self.keyframeInterpolation, curves[10*i+11].keyframe_points, curves[10*i+12].keyframe_points, curves[10*i+13].keyframe_points, modelHandle.boneLocationXFrames[i], modelHandle.boneLocationYFrames[i], modelHandle.boneLocationZFrames[i])
				...
				for i in modelHandle.boneRotationWFrames:
					p2fx_utils.AddKeysList_Rotation(self.keyframeInterpolation, curves[10*i+14].keyframe_points, curves[10*i+15].keyframe_points, curves[10*i+16].keyframe_points, curves[10*i+17].keyframe_points, modelHandle.boneRotationWFrames[i], modelHandle.boneRotationXFrames[i], modelHandle.boneRotationYFrames[i], modelHandle.boneRotationZFrames[i])
					currentFrames += len(modelHandle.boneRotationWFrames[i]) * 4
				updateImportProgress(currentFrames)
				currentFrames = 0
				for i in modelHandle.boneScaleXFrames:
					p2fx_utils.AddKeysList_Location(self.keyframeInterpolation, curves[10*i+18].keyframe_points, curves[10*i+19].keyframe_points, curves[10*i+20].keyframe_points, modelHandle.boneScaleXFrames[i], modelHandle.boneScaleYFrames[i], modelHandle.boneScaleZFrames[i])
			...
			if camData is not None:
				...
				p2fx_utils.AddKeysList_Location(self.keyframeInterpolation, curves[0].keyframe_points, curves[1].keyframe_points, curves[2].keyframe_points, camData.locationXFrames, camData.locationYFrames, camData.locationZFrames)
				p2fx_utils.AddKeysList_Rotation(self.keyframeInterpolation, curves[3].keyframe_points, curves[4].keyframe_points, curves[5].keyframe_points, curves[6].keyframe_points, camData.rotationWFrames, camData.rotationXFrames, camData.rotationYFrames, camData.rotationZFrames)
				p2fx_utils.AddKeysList_Value(self.keyframeInterpolation, curves[7].keyframe_points, camData.lensFrames)
