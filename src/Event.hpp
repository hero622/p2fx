...
#define _ON_INIT1(x)                                    \
	static void _p2fx_init_fn_##x();                        \
	static P2fxInitHandler _p2fx_init_##x(_p2fx_init_fn_##x); \
	static void _p2fx_init_fn_##x()
#define _ON_INIT(x) _ON_INIT1(x)
#define ON_INIT _ON_INIT(__COUNTER__)

#define _ON_EVENT1(ev, x, pri)                                            \
	static void _p2fx_event_fn_##x(Event::EventData<Event::ev> event);        \
	ON_INIT { Event::RegisterCallback<Event::ev>(&_p2fx_event_fn_##x, pri); } \
	static void _p2fx_event_fn_##x(Event::EventData<Event::ev> event)
...
class P2fxInitHandler {
public:
	P2fxInitHandler(std::function<void()> cb);
	std::function<void()> cb;

	static std::vector<P2fxInitHandler *> &GetHandlers();
	static void RunAll();
};
