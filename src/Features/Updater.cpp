...
static bool getLatestVersion(std::string *name, std::string *dlUrl, std::string *pdbUrl, bool allowPre) {
	...
	if (allowPre) {
		std::string err;
		res = json11::Json::parse(request("https://api.github.com/repos/hero622/p2fx/releases"), err);
		if (err != "") {
			return false;
		}
		...
		std::string err;
		res = json11::Json::parse(request("https://api.github.com/repos/hero622/p2fx/releases/latest"), err);
		if (err != "") {
			return false;
		}
	}
